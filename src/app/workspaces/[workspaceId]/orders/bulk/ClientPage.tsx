...
export default function ClientPage({ workspaceId }: { workspaceId: string }) {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto">
      <h1 className="text-3xl font-extrabold text-gray-900 mb-6">CSV 일괄 등록</h1>
      ...
      <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100">
        <div className="mb-6">
          <div className="flex items-end gap-3 mb-4">
            ...
            <Button variant="ghost" onClick={downloadSample} disabled={isProcessing}>
              샘플 다운로드
            </Button>
          </div>
          ...
          {file && !progress && (
            <div className="mb-4 p-3 bg-blue-50 rounded-lg border border-blue-200 text-sm text-blue-800">
              선택된 파일: <strong>{file.name}</strong> ({csvData.length}개 감지)
            </div>
          )}
          ...
          {error && <div className="mb-4 p-3 bg-red-50 text-red-800 rounded-lg border border-red-200 text-sm">{error}</div>}
          {success && <div className="mb-4 p-3 bg-green-50 text-green-800 rounded-lg border border-green-200 text-sm">등록 성공!</div>}
        </div>
        ...
        {csvData.length > 0 && (
          <div className="mb-6 overflow-x-auto">
            <table className="w-full text-sm border-collapse border border-gray-300">
              ...
              <tbody>
                {csvData.map((row, i) => (
                  <tr key={`${row.address} ${row.address_detail} ${i}}`} className="hover:bg-gray-50">
        ...
        <Button onClick={handleSubmit} disabled={csvData.length === 0 || isProcessing} className="w-full">
